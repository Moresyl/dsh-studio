...
在运行环境中检查已发布的 Harness 版本，选择精确版本后点击「安装并验证」，可尝试升级或降级。
切换会停止运行中的 Harness。Studio 在独立目录安装，使用临时官方 Profile 检查集成、
回环 HTTP 启动和 Host 协议，通过后才启用。安装或验证失败时保留原运行时，可从控制面板
重新启动；已有 Profile 和会话保留。

**Studio** 标记表示随应用验证的版本。其他上游版本可能因内部接口变化而无法通过检查，
出现在 npm 列表中不代表一定兼容 Studio 或第三方插件。选择带 Studio 标记的版本可回到
内置兼容性基线。完整离线版可离线安装该基线，选择其他版本需要网络。旧版运行时标记仍可读取，
「修复」以最近一次成功验证的版本为目标。不要手动替换托管运行时目录中的包。

Harness 0.1.2 及之后的版本用每次启动的令牌换取 `SameSite=Strict` 会话 Cookie 来鉴权
Web 界面。Studio 外壳从同一回环站点（`http://127.0.0.1`）伺服自身，内嵌窗口因此能像
浏览器标签页一样保存并发送该 Cookie——不修改 Harness 的任何文件。
...
便携 Agent Preset 使用 `.dshpreset` 文件类型。可以在系统中直接打开、拖放到 Studio 任意位置，
也可以在预设选择器中点击「导入」。Studio 会先校验包清单及其记录的每个 SHA-256，再展示身份、
文件数和解包大小；只有明确确认信任后才导入。拒绝确认或校验失败时，文件与当前 Profile 均不会改变。
